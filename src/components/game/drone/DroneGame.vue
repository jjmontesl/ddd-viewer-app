...
<script>
import { DroneGameProcess } from '@/games/drone/DroneGameProcess.js';
    ...
    if (gameUrl) {
        ...
        fetch(gameUrl).then((response) => {
            response.json().then((data) => {
                  ...
                  // Launch game
                  const gameProcess = new DroneGameProcess(this.getSceneViewer(), gameData);
                  this.getSceneViewer().processes.add(gameProcess);
